...
def plot_rdf(
    ...
    for ax, ((label_a, label_b), g_r) in zip(axes, rdfs.items()):
        ...
        if not np.all(np.isfinite(r)) or not np.all(np.isfinite(g_r)):
            log.warning(f"Found non-finite values in RDF data for {label_a} - {label_b}. Skipping.")
            continue
            
        if len(r) > 50000:  # Prevent too many data points
            log.warning(f"RDF data for {label_a} - {label_b} has {len(r)} points. Downsampling.")
