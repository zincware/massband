...
def compute_cluster_lifetime_correlation_fft(cluster_memberships: list[list[set]], 
...
def compute_cluster_statistics(clusters_over_time: list[list[set]]) -> dict:
    ...
    for frame_clusters in tqdm(clusters_over_time, desc="Computing statistics", leave=False):
    ...
    for frame_clusters in clusters_over_time:
        ...
        if total_particles > 0:
            size_fractions[1].append(sum(len(c) for c in frame_clusters if len(c) == 1) / total_particles)
            size_fractions[2].append(sum(len(c) for c in frame_clusters if len(c) == 2) / total_particles)
            size_fractions['3-5'].append(sum(len(c) for c in frame_clusters if 3 <= len(c) <= 5) / total_particles)
            size_fractions['6+'].append(sum(len(c) for c in frame_clusters if len(c) >= 6) / total_particles)
        else:
...
class ClusterAnalysis(zntrack.Node):
    ...
    def run(self):
        ...
        if not self.structures:
            log.info("No structures provided, grouping by element for atom-based clustering.")
            molecules = {
        ...
        for mol in sorted(molecules.keys()):
            ...
            for idx, (position, cell) in enumerate(tqdm(
                ...
                if minimum_distance is not None:
                    ...
                    connectivity_matrix = np.zeros((len(position), len(position)), dtype=int)
            ...
            lags, correlation = compute_cluster_lifetime_correlation_fft(clusters_over_time)
            ...
            try:
                ...
                if np.sum(valid) > 3:
                    popt, _ = curve_fit(exponential_decay, lags[valid], correlation[valid], 
                                       p0=[1.0, len(lags)/10, 0.0], maxfev=5000)
                    stats['cluster_lifetime_tau'] = popt[1] * self.step  # Convert to frame units
                    log.info(f"Characteristic cluster lifetime: {stats['cluster_lifetime_tau']:.1f} frames")
            except:
            ...
            if stats['all_cluster_sizes']:
                ...
                ax.hist(stats['all_cluster_sizes'], 
            ...
            fig_path = self.figures_path / f"cluster_size_distribution_{mol.replace('/', '_')}.png"
            ...
            ax.legend()
            ax.grid(True, alpha=0.3)
            fig_path = self.figures_path / f"n_clusters_over_time_{mol.replace('/', '_')}.png"
            fig.savefig(fig_path, dpi=300, bbox_inches="tight")
            plt.close(fig)
            
            # 3. Mean cluster size over time with running average
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.plot(frames_idx, stats['mean_cluster_size'], 
                   alpha=0.3, color='gray', label='Raw data')
            
            if len(stats['mean_cluster_size']) > self.window_size:
                running_avg = signal.convolve(stats['mean_cluster_size'], 
                                             np.ones(self.window_size)/self.window_size, 
                                             mode='valid')
                avg_frames = frames_idx[:len(running_avg)]
                ax.plot(avg_frames, running_avg, 
            ...
            ax.legend()
            ax.grid(True, alpha=0.3)
            fig_path = self.figures_path / f"mean_cluster_size_over_time_{mol.replace('/', '_')}.png"
            fig.savefig(fig_path, dpi=300, bbox_inches="tight")
            plt.close(fig)
            ...
            if stats['cluster_lifetime_tau'] is not None:
                ...
                fit_y = exponential_decay(lags, 1.0, stats['cluster_lifetime_tau']/self.step, 0)
                ax.plot(lags * self.step, fit_y, '--', 
            ...
            if stats['cluster_lifetime_tau'] is not None:
                ax.legend()
            fig_path = self.figures_path / f"cluster_lifetime_correlation_{mol.replace('/', '_')}.png"
            fig.savefig(fig_path, dpi=300, bbox_inches="tight")
            plt.close(fig)
            ...
            for size_key, fractions in stats['size_fractions'].items():
                if len(fractions) > self.window_size:
                    running_avg = signal.convolve(fractions, 
                                                 np.ones(self.window_size)/self.window_size, 
                                                 mode='valid')
                    avg_frames = frames_idx[:len(running_avg)]
                    ax.plot(avg_frames, running_avg, 
                           linewidth=2, label=f'Size {size_key}')
            ...
            ax.grid(True, alpha=0.3)
            ax.set_ylim(0, 1)
            fig_path = self.figures_path / f"cluster_size_fractions_{mol.replace('/', '_')}.png"
            fig.savefig(fig_path, dpi=300, bbox_inches="tight")
            plt.close(fig)
            ...
            ax.axhline(y=0.5, color='r', linestyle='--', 
                      label='Percolation threshold (50%)')
            ax.set_xlabel("Frame", fontsize=12)
            ...
            ax.grid(True, alpha=0.3)
            ax.set_ylim(0, 1)
            fig_path = self.figures_path / f"percolation_indicator_{mol.replace('/', '_')}.png"
            fig.savefig(fig_path, dpi=300, bbox_inches="tight")
            plt.close(fig)
            
            # Save numerical data
            np.savez(self.data_path / f"cluster_analysis_{mol.replace('/', '_')}.npz",
                    ...
                    cluster_lifetime_tau=stats['cluster_lifetime_tau'] if stats['cluster_lifetime_tau'] else np.nan,
