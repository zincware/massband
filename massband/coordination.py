...
class CoordinationNumber(zntrack.Node):
    ...
    max_integration_distance: float = zntrack.params(10.0)  # Maximum distance to consider (Å)
    ...
    def _calculate_coordination_number(self, r: np.ndarray, g_r: np.ndarray, 
                                     number_density: float, first_min_distance: float) -> float:
    ...
    def _plot_coordination_analysis(self, pair_name: str, r: np.ndarray, g_r: np.ndarray,
                                  first_min_distance: float, coordination_number: float,
                                  number_density: float):
        ...
        ax1.axvline(first_min_distance, color='red', linestyle='--', 
                   label=f'1st minimum: {first_min_distance:.2f} Å')
        ax1.fill_between(r[r <= first_min_distance], 
                        g_r[r <= first_min_distance], 
                        alpha=0.3, color='green',
                        label='Integration region')
        ax1.axhline(self.density_threshold, color='gray', linestyle=':', 
        ...
        ax2.axhline(coordination_number, color='blue', linestyle='-',
    ...
    def run(self):
        ...
        log.info(f"Calculating coordination numbers for {len(self.rdf.results)} RDF pairs")
        ...
        for pair_key, g_r_list in self.rdf.results.items():
            ...
            self._plot_coordination_analysis(pair_key, r, g_r, 
                                           first_min_distance, coordination_number,
                                           estimated_number_density)
            
            log.info(f"{pair_key}: CN = {coordination_number:.2f}, "
